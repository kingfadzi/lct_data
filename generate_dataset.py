# Script 1: generate_dataset.py
# This script connects to your database using settings from config.yaml,
# runs the SQL to build the edges dataset, and writes the result to a CSV for downstream consumption.
...
WITH
  base AS (
    ...
    FROM
      public.vwsfitserviceinstance AS si
      JOIN public.lean_control_application AS fia ON fia.servicenow_app_id = si.correlation_id
      JOIN public.lean_control_product_backlog_details AS lpbd ON lpbd.lct_product_id = fia.lean_control_service_id
      JOIN public.vwsfbusinessapplication AS bac ON si.business_application_sysid = bac.business_application_sys_id
      LEFT JOIN public.vwsfbusinessapplication AS bap ON bac.application_parent_correlation_id = bap.correlation_id
  ),
  ...
  edges AS (
    ...
    SELECT
      ...
      ds.it_business_service AS name,
...
ORDER BY
  CASE
    WHEN id = 'Business Services' THEN 0
    ELSE 1
  END,
  parent,
  id;
